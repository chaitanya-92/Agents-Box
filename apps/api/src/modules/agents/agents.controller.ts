  ...
  try {
    ...
    await prisma.agentUsage.create({
      data: {
        userId: req.user.id,
        agentId: agent.id,
        metadata: JSON.stringify({ source: "api", invokedAt: new Date().toISOString() })
      }
    });
    return res.status(StatusCodes.OK).json(
      success({ agent, response: `Stubbed execution for ${agent.name}. Replace with provider-specific orchestration.` })
    );
  } catch (err) { next(err); }
}
